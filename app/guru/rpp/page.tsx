...
export default async function MyRppPage() {
  ...
  // Load semua RPP milik guru; filtering dilakukan instant di sisi client.
  ...
  return (
    <div>
      ...
      {/* Pencarian instant No. RPP / Materi / Mapel / Kelas */}
      {rppList.length === 0 ? (
        <Card className="p-10 text-center border-dashed">
          ...
          <p className="text-sm text-muted-foreground">
            Belum ada RPP. Klik &quot;Buat RPP Baru&quot; untuk memulai.
          </p>
          {!guruId && (
        ...
        <div>
          <RppListSearch containerId="rpp-list" placeholder="Cari No. RPP, materi, mapel, kelas…" />
          <div id="rpp-list" className="space-y-3">
            {rppList.map((r) => (
              <Card
                key={r.id}
                data-row
                data-search={`${r.noRpp ?? ""} ${r.materi} ${r.mapel.namaMapel} ${r.kelas.namaKelas} ${STATUS_LABEL[r.status] ?? ""}`}
